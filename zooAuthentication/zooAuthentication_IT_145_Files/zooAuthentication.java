...
class prompts{
    ...
    public static void loginSuccessPrompt() {
    ...
    public static void loginAttemptsPrompts() {
...
public class zooAuthentication 
{
    public static void main(String[] args) throws Exception {
        ...
        userInputs uIn = new userInputs();
        prompts prompts = new prompts();

		/* Updated previous file type from .txt to .csv to read from new csv file; 	*/	
		/* Path should be updated on local system to correct location 	*/
		
        uIn.filesPath = ("C:\\Users\\miguel.ovalles\\Downloads\\zooAuthentication\\");
        uIn.credentialsFile = ("credentials.csv");															 
        ...
        while(true) 
        {
            ...
            if(!(uIn.userName.contentEquals("quit")))  { 															//If username not equal to quit, then continue program.
                ...
                while(credentialCheck.hasNextLine()){
					...
					/* Removed ("\t"); and replaced it with (","); for .csv in order to read from new csv credentials file */
					
                    String colLocation[] = credReview.split(",");  		
					
                    /* .csv columns are as follows: name, hashPassword, password, role */
                    ...
                    if(colLocation[0].trim().equals(uIn.userName)){
                        ...
                        if(colLocation[1].trim().equals(sb.toString())){
                            ...
                            Scanner userRole = new Scanner(new File(uIn.filesPath + colLocation[3].trim() + ".txt"));           //Scanner to go to file path and read credentials file, index 3 for user role. - Updated to csv from txt
